...
if Norm_type == 1
    norm = 1;  % Fixed normalization value
elseif Norm_type == 2
    maxAmplitudes = cellfun(@max, amplitudes);
    [~, idxMinMax] = min(maxAmplitudes);
    % Pega o valor central (posição do meio) da curva correspondente
    curva = amplitudes{idxMinMax};
    centralIndex = ceil(length(curva)/2);
    norm = curva(centralIndex);
else
