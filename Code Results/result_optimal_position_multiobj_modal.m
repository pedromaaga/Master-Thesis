...
pareto_optimal_points_x_perc = struct();
pareto_optimal_points_of = struct();
...
for k = 1:length(keys_of)
    ...
    pareto_optimal_points_x_perc.(key).harmonic_mean = pareto_points_x_perc.(key){idx_hm};
    pareto_optimal_points_x_perc.(key).euclidean_mean = pareto_points_x_perc.(key){idx_ed_mean};
    pareto_optimal_points_x_perc.(key).euclidean_median = pareto_points_x_perc.(key){idx_ed_median};
    pareto_optimal_points_x_perc.(key).minmax = pareto_points_x_perc.(key){idx_minmax};
    pareto_optimal_points_x_perc.(key).maxmin = pareto_points_x_perc.(key){idx_maxmin};

    pareto_optimal_points_of.(key).harmonic_mean = pareto_points_of.(key){idx_hm};
    pareto_optimal_points_of.(key).euclidean_mean = pareto_points_of.(key){idx_ed_mean};
    pareto_optimal_points_of.(key).euclidean_median = pareto_points_of.(key){idx_ed_median};
    pareto_optimal_points_of.(key).minmax = pareto_points_of.(key){idx_minmax};
    pareto_optimal_points_of.(key).maxmin = pareto_points_of.(key){idx_maxmin};
end
...
% Ordem fixa das labels
labels = {'harmonic_mean', 'euclidean_mean', 'euclidean_median', 'minmax', 'maxmin'};
legend_entries = {'Harmônica', 'Euclidiana - Média', 'Euclidiana - Mediana', 'MinMax', 'MaxMin'};
markers = {'*', 'x', '+', 's', '^'};
colors = {[0.85 0.33 0.10], [0.00 0.45 0.74], [0.47 0.67 0.19], [0.49 0.18 0.56], [0.30 0.75 0.93]};

% Mapear label → marcador e cor
label_to_marker = containers.Map(labels, markers);
label_to_color = containers.Map(labels, colors);
...
for k = 1:length(keys_of)
    ...
    for i = 1:length(labels)
        ...
        plot3(obj_val(1), obj_val(2), obj_val(3), marker, ...
              'MarkerSize', 8, 'LineWidth', 1.5, 'Color', color);
    end
    ...
    for c = 1:3
        ...
        plot(frontier_3d_proj(:, 1), frontier_3d_proj(:, 2), '.', 'Color', [0.6 0.6 0.6], 'MarkerSize', 8);
        plot(frontier_2d(:, 1), frontier_2d(:, 2), 'ro', 'LineWidth', 1.5);
        ...
        for i = 1:length(labels)
            ...
            plot(proj_pt(1), proj_pt(2), marker, ...
                 'MarkerSize', 8, 'LineWidth', 1.5, 'Color', color);
        end
